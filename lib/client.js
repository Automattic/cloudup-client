...
function Cloudup(options) {
  ...
  this.ua = (options.useragent || 'cloudup-client') + ua;
...
Cloudup.prototype.requestToken = function(appId, fn){
  if (!this.user) return fn(new Error('.user required to request a token'));
  if (!this.pass) return fn(new Error('.pass required to request a token'));
