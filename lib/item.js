...
Item.prototype.create = function(len, fn){
  ...
  dimensions(path, function(err, dims){
    ...
    self.client
    .post('/items')
    .set('X-Cloudup-Required-Fields', '1')
    .send(item)
    .end(function(err, res){
      if (err) return fn(err);
      if (res.error) return fn(error(res));
...
Item.prototype.form = function(key){
  ...
  // append any form fields required by the server-side
  var required = this.s3_required_form_fields;
  if (required) {
    for (var i in required) {
      debug('appending required form field "%s": %s', i, required[i]);
      form.append(i, required[i]);
    }
  }
  return form;
};
...
Item.prototype.postURL = function(fn){
  ...
  this.client
  .post('/items/url')
  .send({ url: url, bucket: 'cldup', stream_id: stream.id })
  .end(function(err, res){
    if (err) return fn(err);
    if (res.error) return fn(error(res));
    self.merge(res.body);
